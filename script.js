...
class Effect {
  ...
  createPointElement() {
    ...
    function createPointParticles({ texture, size, total, transparent = true, max = 150, min = 70, pointY }) {
      ...
      let blending = transparent ? THREE.AdditiveBlending : THREE.NormalBlending;
  ...
  updateNucleus() {
    ...
    for (let i = 0; i < this.nucleusPosition.count; i++) {
      ...
      const distanceNucleus =
        20 +
        this.noise(nxNucleus + this.time * 0.0004, nyNucleus + this.time * 0.0004) * this.blobScale * animationEasing1;
  ...
  updateSinglePointStarContraction(positions, i, easing) {
    ...
    currentDistancePointContr = Math.sqrt(
      xPointContr * xPointContr + yPointContr * yPointContr + zPointContr * zPointContr
    );
